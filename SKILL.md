# agents-cfgit：AI 工具配置文件版本控制

## 0. 这是什么

agents-cfgit 是一个基于 Git 的版本控制系统，专门管理你的 AI 工具配置文件。
...
系统正常工作时有以下特征。**推荐直接用 `agents-cfgit verify` 一键验证**（跨平台）：
...
claude (C:\Users\xxx\.claude):
  ...
  ✅ claude: hook 注册 — 已注册
  ✅ claude: SKILL.md — C:\Users\xxx\.claude\skills\agents-cfgit\SKILL.md

✅ 全部检查通过
```

任何 ❌ 都说明安装不完整，请告知用户执行 `agents-cfgit init` 修复。
...
当用户问"备份正常吗"或"agents-cfgit 可用吗"时，**优先用 `agents-cfgit verify` 一键检查**：
...
任何 ❌ 都提示用户重新执行 `agents-cfgit init` 或查看 5.3 节故障排查。
...
当用户问"备份正常吗"时，**优先用 `agents-cfgit status`**（跨平台一键检查）：
...
如果用户问"磁盘占用多大"，提示：**agents-cfgit 不跟踪磁盘占用**（仅是个 git 仓库）。
如需查看可用以下命令：
...
**原因：** agents-cfgit 未初始化或 .git 目录被误删。
...
agents-cfgit init
```
然后告知用户："agents-cfgit 已重新初始化，首次 commit 已创建。"
...
1. **先用 `agents-cfgit verify` 一键检查 hook 是否注册**：
...
`agents-cfgit squash` 每月 1-3 号执行一次，连续触发确保至少命中一次开机日。

**告知用户：**
```
agents-cfgit 会自动压缩 90 天前的 commit 以保持历史整洁。
...
**手动触发 squash（仅调试用）：**
```bash
agents-cfgit squash           # 使用默认 90 天阈值
agents-cfgit squash --days 30 # 自定义阈值
```
...
- **如果修改了 settings.json 导致 hooks 不生效**，重新执行 `agents-cfgit init`
- **如需长期保留关键版本**，告知 LLM 对某个 commit 打 tag
- 系统按设计每月 1-3 号自动压缩 90 天前的 commit（详见 `agents-cfgit squash`）

---

## 7. 架构说明（给高级用户）

### 7.1 为什么 agents-cfgit 不打包成 plugin？
...
4 个 agent 中只有 1 个支持 plugin 声明 hook（Claude Code），其他 3 个必须直接写 hook 配置文件。**为了跨 agent 一致性，agents-cfgit 选择统一的"硬编码 + 增量卸载"方案**，卸载时通过 `enabledPlugins.xxx = false` 等效操作清理。

如未来 Claude Code plugin 机制更成熟，可考虑为 Claude 单独提供 plugin 打包方式。

### 7.2 卸载为什么不是"配置项开关"？

理论上 Claude Code 的 `enabledPlugins: { "agents-cfgit@xxx": false }` 即可禁用。但：
- Cursor / Codex / OpenCode 都没有"插件开关"机制
- 即使 Claude 用了 plugin 机制，其他 3 个仍需"硬卸载"
- 当前 `agents-cfgit uninstall` 已实现"一个命令卸载所有"，对用户体验无差异
...
- `agents-cfgit verify` 只能检查**文件层面**状态，无法检查**进程内缓存**状态

这是 AI 工具的设计限制，agents-cfgit 无法绕过。请始终遵循 `UNINSTALL.md` 中的"重启 AI 工具会话"步骤。
